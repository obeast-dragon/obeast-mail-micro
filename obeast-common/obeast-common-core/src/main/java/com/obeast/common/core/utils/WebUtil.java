...
import com.obeast.common.core.exception.ObeastException;
...
public class WebUtil extends org.springframework.web.util.WebUtils {
	...
	public boolean isBody(HandlerMethod handlerMethod) {
		ResponseBody responseBody = ClazzUtil.getAnnotation(handlerMethod, ResponseBody.class);
	...
	private static String[] splitClient(String header) {
		if (header == null || !header.startsWith(BASIC_)) {
			throw new ObeastException("请求头中client信息为空");
		}
		...
		try {
			...
			throw new ObeastException("Failed to decode basic authentication token");
		}
		...
		if (delim == -1) {
			throw new ObeastException("Invalid basic authentication token");
		}
