package com.obeast.common.feign.sentinel.handle;
...
public class GlobalBizExceptionHandler {
	...
	public CommonResult<?> handleBodyValidException(MethodArgumentNotValidException exception) {
		...
		return CommonResult.error(String.format("%s %s", fieldErrors.get(0).getField(), fieldErrors.get(0).getDefaultMessage()));
	}
	...
	public CommonResult<?> bindExceptionHandler(BindException exception) {
		List<FieldError> fieldErrors = exception.getBindingResult().getFieldErrors();
		log.warn("参数绑定异常,ex = {}", fieldErrors.get(0).getDefaultMessage());
		return CommonResult.error(fieldErrors.get(0).getDefaultMessage());
	}
